# diabetes-prediction-v3-optimized.py
import pandas as pd
import numpy as np
import warnings
...
# ==================== 1. DATA LOADING ====================
print("Loading data...")
dataset = pd.read_csv(r"/kaggle/input/diabetes-health-indicators-dataset/diabetes_dataset.csv")
...
extra_features = [col for col in dataset.columns if col not in train_data.columns and col != "id"]
common_features = [col for col in dataset.columns if col in train_data.columns and col != "diagnosed_diabetes"]

imputation_models = {}

print("-" * 10 + " Training Imputation Models " + "-" * 10)
for feature in extra_features:
    if feature not in dataset.columns: continue

    mask = dataset[feature].notna()
    if mask.sum() < 50: continue  # Skip if too little data
    ...
    if is_categorical:
        ...
        model = RandomForestClassifier(n_estimators=50, max_depth=8, n_jobs=-1, random_state=SEED)
        model.fit(X_encoded, y_encoded)
        encoders["target"] = le_target
    else:
        model = RandomForestRegressor(n_estimators=50, max_depth=8, n_jobs=-1, random_state=SEED)
...
def safe_impute(data, models, common_features):
    ...
    for feature, model_dict in models.items():
        ...
        for col, encoder in encoders.items():
            if col == "target" or col not in X.columns: continue
            ...
            X[col] = X[col].apply(lambda x: x if x in known_labels else encoder.classes_[0])
...
def feature_engineering(data, is_train=True):
    ...
    if 'bmi' in df.columns:
        ...
        df['bmi_cat'] = pd.cut(df['bmi'], bins=[-1, 18.5, 25, 30, 100], labels=['Under', 'Normal', 'Over', 'Obese'])
        
    # if {'glucose_fasting','insulin_level'}.issubset(df.columns):
    #     G = df['glucose_fasting'].clip(lower=1.0)
    #     I = df['insulin_level'].clip(lower=1.0)
        
    #     df['homa_ir'] = (G * I) / 405 
    #     df['g_i_ratio'] = G / I
    
    # if {'cholesterol','hdl_cholesterol'}.issubset(df.columns):
    #     HDL = df['hdl_cholesterol'].clip(lower=1.0)
    #     df['athero_index'] = (df['cholesterol'] - HDL) / HDL
...
X_train_fe = pd.concat([X_train_fe, X_dataset_fe], axis=1)
...
if 'id' in X_train_fe.columns: X_train_fe.drop('id', axis=1, inplace=True)
if 'id' in X_test_fe.columns: X_test_fe.drop('id', axis=1, inplace=True)
...
if 'cluster_id' in X.columns: cat_cols.append('cluster_id')
if 'age_group' in X.columns: cat_cols.append('age_group')
